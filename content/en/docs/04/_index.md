...
* `helm install --dry-run=server --debug --generate-name <chart> --namespace $USER` or `helm template --debug <chart>`: We’ve seen this trick already. It’s a great way to render your templates without applying them to the cluster.
...
When your YAML is failing to parse but you want to see what is generated, one easy way to retrieve the YAML is to comment out the problem section in the template and then re-run `helm install --dry-run=server --debug --generate-name <chart>`.
...
```bash
git clone https://github.com/acend/error-chart.git && cd error-chart 
```
